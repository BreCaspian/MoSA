...
void  DynObjFilter::init(rclcpp::Node& node)
{
    ...
    DeclareAndGet<bool>(node, "dyn_obj/dyn_points_stable_en", dyn_points_stable_en, true);
    DeclareAndGet<int>(node, "dyn_obj/dyn_points_accu_limit", dyn_points_accu_limit, 3);
    DeclareAndGet<float>(node, "dyn_obj/dyn_points_voxel", dyn_points_voxel, 0.4f);
    ...
    DeclareAndGet<bool>(node, "dyn_obj/cluster_async", cluster_async, false);
    DeclareAndGet<int>(node, "dyn_obj/cluster_queue_size", cluster_queue_size, 2);
    cluster_queue_size = std::max(1, cluster_queue_size);
    ...
    DeclareAndGet<bool>(node, "dyn_obj/track_enable", Cluster.track_enable, true);
    DeclareAndGet<int>(node, "dyn_obj/track_min_hits", Cluster.track_min_hits, 3);
    DeclareAndGet<int>(node, "dyn_obj/track_max_age", Cluster.track_max_age, 5);
    DeclareAndGet<int>(node, "dyn_obj/track_min_points", Cluster.track_min_points, 30);
    DeclareAndGet<float>(node, "dyn_obj/track_gate", Cluster.track_gate, 1.5f);
    DeclareAndGet<float>(node, "dyn_obj/aabb_alpha", Cluster.aabb_alpha, 0.6f);
    ...
    Cluster.Init();
    cloud_pool_max = std::max(32, dyn_points_accu_limit * 4 + laserCloudSteadObj_accu_limit * 2);
    cloud_pool.clear();
    cloud_pool.resize(static_cast<size_t>(cloud_pool_max));
    for(int i = 0; i < cloud_pool_max; ++i)
    {
        cloud_pool[i].reset(new PointCloudXYZI());
    }
    if(cluster_async)
    {
        if(cluster_coupled || cluster_future)
        {
            RCLCPP_WARN(node.get_logger(), "cluster_async enabled, cluster_coupled/cluster_future will be disabled.");
...
void  DynObjFilter::filter(PointCloudXYZI::Ptr feats_undistort, const M3D & rot_end, const V3D & pos_end, const double & scan_end_time)
{
    ...
    if(!laserCloudDynObj) laserCloudDynObj.reset(new PointCloudXYZI());
    laserCloudDynObj->clear();
    laserCloudDynObj->reserve(size);
    if(!laserCloudDynObj_world) laserCloudDynObj_world.reset(new PointCloudXYZI());
    laserCloudDynObj_world->clear();
    laserCloudDynObj_world->reserve(size);
    if(!laserCloudSteadObj) laserCloudSteadObj.reset(new PointCloudXYZI());
    laserCloudSteadObj->clear();
    laserCloudSteadObj->reserve(size);
    if(!laserCloudDynObj_clus) laserCloudDynObj_clus.reset(new PointCloudXYZI());
    laserCloudDynObj_clus->clear();
    laserCloudDynObj_clus->reserve(size);
    if(!laserCloudSteadObj_clus) laserCloudSteadObj_clus.reset(new PointCloudXYZI());
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, index_cache.size(), 4096), [&](const tbb::blocked_range<size_t> &range) {
        for(size_t idx = range.begin(); idx != range.end(); ++idx)
        {
            int i = index_cache[idx];
            p[i].reset();     
    ...
    if (cluster_coupled || cluster_future || cluster_async)
    {
        PointCloudXYZI::Ptr cluster_input = laserCloudDynObj;
        if(dyn_points_stable_en)
        {
            cluster_input = StabilizeDynPoints(laserCloudDynObj, dyn_points_accu_cluster_in);
        }
        if(cluster_async)
        {
            ClusterJob job;
            if(dyn_points_stable_en)
            {
                job.dyn_points = cluster_input;
            }
            else
            {
                job.dyn_points = CopyCloudToPool(cluster_input);
            }
            job.raw_points = AcquireCloud();
            job.raw_points->swap(raw_points_world);
            job.header = header_clus;
            job.rot = rot_end;
            job.pos = pos_end;
            {
                std::lock_guard<std::mutex> lock(cluster_mtx);
                if(static_cast<int>(cluster_queue.size()) >= cluster_queue_size)
                {
                    ClusterJob drop = std::move(cluster_queue.front());
                    cluster_queue.pop_front();
                    ReleaseCloud(drop.dyn_points);
                    ReleaseCloud(drop.raw_points);
                }
                cluster_queue.push_back(std::move(job));
            }
            cluster_cv.notify_one();
        }
        else
        {
            Cluster.Clusterprocess(dyn_tag_cluster, *cluster_input, raw_points_world, header_clus, rot_end, pos_end);
            if(dyn_points_stable_en)
            {
                ReleaseCloud(cluster_input);
            }
            for(int i = 0; i < size; i++)
            {   
                PointType po;
    ...
    Points2Buffer(points, index_cache);
...
void  DynObjFilter::Points2Buffer(vector<point_soph*> &points, std::vector<int> &index_vector)
{
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, index_vector.size(), 4096), [&](const tbb::blocked_range<size_t> &range) {
        for(size_t idx = range.begin(); idx != range.end(); ++idx)
        {
...
void  DynObjFilter::Buffer2DepthMap(double cur_time)
{
    ...
    double t = 0.0;
    int max_point = 0;
    if(depth_bucket_lookup.empty())
    {
        depth_bucket_lookup.assign(MAX_2D_N, -1);
        depth_bucket_positions.reserve(4096);
        depth_bucket_points.reserve(4096);
    }
    for (int k = 0; k < len; k++)
    {   
        ...
        {   
            ...
            map = depth_map_list.back();
            switch (point->dyn)
            {
                if(map->depth_map.size() <= point->position) 
                case STATIC:
                    SphericalProjection(*point, map->map_index, map->project_R, map->project_T, *point);
                    map->MarkDirty(point->position);
                    if(point->position >= 0 && point->position < MAX_2D_N)
                    {
                        int &bucket_idx = depth_bucket_lookup[point->position];
                        if(bucket_idx < 0)
                        {
                            bucket_idx = static_cast<int>(depth_bucket_points.size());
                            depth_bucket_positions.push_back(point->position);
                            depth_bucket_points.emplace_back();
                            depth_bucket_points.back().reserve(8);
                        }
                        depth_bucket_points[bucket_idx].push_back(point);
                    }
                    break;   
                case CASE1:   
    ...
    if(!depth_bucket_positions.empty() && !depth_map_list.empty())
    {
        DepthMap::Ptr map = depth_map_list.back();
        for(size_t bi = 0; bi < depth_bucket_positions.size(); ++bi)
        {
            int pos = depth_bucket_positions[bi];
            std::vector<point_soph*> &points = depth_bucket_points[bi];
            if(points.empty()) continue;
            if(map->depth_map[pos].empty() && map->depth_map[pos].capacity() < 8)
            {
                map->depth_map[pos].reserve(std::min(8, max_pixel_points));
            }
            size_t base = map->depth_map[pos].size();
            for(size_t i = 0; i < points.size(); ++i)
            {
                point_soph* point = points[i];
                if(map->depth_map[pos].size() >= static_cast<size_t>(max_pixel_points))
                {
                    break;
                }
                map->depth_map[pos].push_back(point);
                const size_t idx = base + i;
                if(point->vec(2) > map->max_depth_all[pos])
                {
                    map->max_depth_all[pos] = point->vec(2);
                    map->max_depth_index_all[pos] = static_cast<int>(idx);
                }
                if(point->vec(2) < map->min_depth_all[pos] || map->min_depth_all[pos] < 10E-5)
                {
                    map->min_depth_all[pos] = point->vec(2);
                    map->min_depth_index_all[pos] = static_cast<int>(idx);
                }
                if(point->dyn == STATIC)
                {
                    if(point->vec(2) < map->min_depth_static[pos] || map->min_depth_static[pos] < 10E-5)
                    {
                        map->min_depth_static[pos] = point->vec(2);
                    }
                    if(point->vec(2) > map->max_depth_static[pos])
                    {
                        map->max_depth_static[pos] = point->vec(2);
                    }
                }
            }
            points.clear();
            depth_bucket_lookup[pos] = -1;
        }
        depth_bucket_positions.clear();
    }
    if (debug_en)
    {   
...
{
    if(p.last_proj_index == depth_index)
    {
        p_spherical.vec = p.last_proj_vec;
        p_spherical.hor_ind = p.last_proj_pos[0];
        p_spherical.ver_ind = p.last_proj_pos[1];
        p_spherical.position = p.last_proj_pos[2];
        return;
    }
    ...
    p.last_proj_index = depth_index;
    p.last_proj_vec = p_spherical.vec;
    p.last_proj_pos[0] = p_spherical.hor_ind;
    p.last_proj_pos[1] = p_spherical.ver_ind;
    p.last_proj_pos[2] = p_spherical.position;
}
...
{
    const float p_x = p.vec(0);
    const float p_y = p.vec(1);
    const float p_z = p.vec(2);
    const double p_time = p.time;
    const int hor_ind = p.hor_ind;
    const int ver_ind = p.ver_ind;
    const DepthMap2D &depth_map = map_info.depth_map;
    float all_minus = 0;
    ...
    for (int ind_hor = -depth_cons_hor_num2; ind_hor <= depth_cons_hor_num2; ind_hor ++)
    {
        ...
        {   
            int pos_new = ((hor_ind + ind_hor)%MAX_1D) * MAX_1D_HALF + ((ver_ind + ind_ver)%MAX_1D_HALF); 
            if (pos_new < 0 || pos_new >= MAX_2D_N)  continue;
            const vector<point_soph*> & points_in_pixel = depth_map[pos_new];
            for (int j = 0; j < points_in_pixel.size(); j ++)
            {
                const point_soph* point = points_in_pixel[j]; 
                if(fabs(point->time - p_time) < frame_dur && fabs(point->vec(0)-p_x) < depth_cons_hor_thr2 && \
                  fabs(point->vec(1)-p_y) < depth_cons_ver_thr2)
                {
                    all_num ++;
                    if (point->dyn == STATIC) 
                    {
                        float cur_minus = p_z-point->vec(2);
                        if (fabs(cur_minus) < depth_cons_depth_max_thr2)
                        {
                            num ++;
                            all_minus += fabs(point->vec(2)-p_z);
                        }
                        else if (cur_minus > 0)
                        {
                            smaller_num ++;
    ...
    if(all_num > 0)
    {
        if(num > 1)
        {           
            float cur_depth_thr = max(depth_cons_depth_thr2, k_depth2*p_z);
...
bool  DynObjFilter::Case3DepthConsistencyCheck(const point_soph & p, const DepthMap &map_info)
{
    ...
    for (int ind_hor = -depth_cons_hor_num3; ind_hor <= depth_cons_hor_num3; ind_hor ++)
    {
        ...
        {   
            ...
            {
                const point_soph* point = points_in_pixel[j]; 
                if(fabs(point->time - p_time) < frame_dur && fabs(point->vec(0)-p_x) < depth_cons_hor_thr3 && \
                  fabs(point->vec(1)-p_y) < depth_cons_ver_thr3)
                {
                    all_num ++;
                    if (point->dyn == STATIC) 
                    {
    ...
    if(all_num > 0)
    {
        if(num > 1)
        {      
            float cur_depth_thr = max(depth_cons_depth_thr3, k_depth3*p_z);
            if(all_minus/(num-1) > cur_depth_thr)
            {
                return false;
            }      
                               ...
                               const double & scan_end_time)
{
    ...
    case2_num = 0;
    case3_num = 0;
    PointCloudXYZI::Ptr dyn_points_pub = laserCloudDynObj_world;
    if(dyn_points_stable_en)
    {
        dyn_points_pub = StabilizeDynPoints(laserCloudDynObj_world, dyn_points_accu_raw);
    }
    sensor_msgs::msg::PointCloud2 laserCloudFullRes3;
    pcl::toROSMsg(*dyn_points_pub, laserCloudFullRes3);
    ...
    if(dyn_points_stable_en)
    {
        ReleaseCloud(dyn_points_pub);
    }
    if(cluster_coupled || cluster_future)
    {
        PointCloudXYZI::Ptr dyn_frame_pub = laserCloudDynObj_clus;
        if(dyn_points_stable_en)
        {
            dyn_frame_pub = StabilizeDynPoints(laserCloudDynObj_clus, dyn_points_accu_cluster_out);
        }
        sensor_msgs::msg::PointCloud2 laserCloudFullRes4;
        pcl::toROSMsg(*dyn_frame_pub, laserCloudFullRes4);
    ...
    if(cluster_coupled)
    {
        if(laserCloudSteadObj_accu_times < laserCloudSteadObj_accu_limit)
        {
            ...
            laserCloudSteadObj_accu.push_back(CopyCloudToPool(laserCloudSteadObj_clus));
        ...
        {   
            ReleaseCloud(laserCloudSteadObj_accu.front());
    ...
    else
    {
        ...
        {
            laserCloudSteadObj_accu_times ++;
            laserCloudSteadObj_accu.push_back(CopyCloudToPool(laserCloudSteadObj));
            for(int i = 0; i < laserCloudSteadObj_accu.size(); i++)
            {
                *laserCloudSteadObj_pub += *laserCloudSteadObj_accu[i];
            }
...
PointCloudXYZI::Ptr DynObjFilter::StabilizeDynPoints(const PointCloudXYZI::Ptr &src,
                                                     std::deque<PointCloudXYZI::Ptr> &buffer)
{
    ...
    if(dyn_points_accu_limit > 1)
    {
        buffer.push_back(CopyCloudToPool(src));
        if(static_cast<int>(buffer.size()) > dyn_points_accu_limit)
        {
            ReleaseCloud(buffer.front());
            buffer.pop_front();
        }
    }
    else
    {
        return CopyCloudToPool(src);
    }
    PointCloudXYZI::Ptr merged(AcquireCloud());
    size_t total = 0;
    ...
    if(dyn_points_voxel > 1e-6f)
    {
        pcl::VoxelGrid<PointType> voxel;
        voxel.setLeafSize(dyn_points_voxel, dyn_points_voxel, dyn_points_voxel);
        voxel.setInputCloud(merged);
        PointCloudXYZI::Ptr filtered(AcquireCloud());
        filtered->clear();
...
PointCloudXYZI::Ptr DynObjFilter::AcquireCloud()
{
    {
        std::lock_guard<std::mutex> lock(cloud_pool_mtx);
        if(!cloud_pool.empty())
        {
            PointCloudXYZI::Ptr cloud = cloud_pool.front();
            cloud_pool.pop_front();
            return cloud;
        }
    }
...
void DynObjFilter::ReleaseCloud(PointCloudXYZI::Ptr cloud)
{
    if(!cloud)
    {
        return;
    ...
    cloud->clear();
    std::lock_guard<std::mutex> lock(cloud_pool_mtx);
    if(static_cast<int>(cloud_pool.size()) >= cloud_pool_max)
    {
        return;
    }
    cloud_pool.push_back(cloud);
}

PointCloudXYZI::Ptr DynObjFilter::CopyCloudToPool(const PointCloudXYZI::Ptr &src)
{
    PointCloudXYZI::Ptr dst = AcquireCloud();
    dst->clear();
    if(src)
    {
...
void DynObjFilter::StartClusterThread()
{
    if(cluster_thread_running)
    {
        return;
    }
    cluster_thread_running = true;
    cluster_thread = std::thread(&DynObjFilter::ClusterWorker, this);
}

void DynObjFilter::StopClusterThread()
{
    if(!cluster_thread_running)
    {
        return;
    ...
    {
        std::lock_guard<std::mutex> lock(cluster_mtx);
        cluster_thread_running = false;
    }
    cluster_cv.notify_all();
    if(cluster_thread.joinable())
    {
        cluster_thread.join();
    }
}

void DynObjFilter::ClusterWorker()
{
    while(true)
    {
        ClusterJob job;
        {
            std::unique_lock<std::mutex> lock(cluster_mtx);
            cluster_cv.wait(lock, [&] { return !cluster_queue.empty() || !cluster_thread_running; });
            if(!cluster_thread_running && cluster_queue.empty())
            {
                break;
            }
            job = std::move(cluster_queue.front());
            cluster_queue.pop_front();
        }
        if(job.dyn_points && job.raw_points)
        {
            dyn_tag_worker_cache.assign(job.raw_points->size(), 0);
            Cluster.Clusterprocess(dyn_tag_worker_cache, *job.dyn_points, *job.raw_points, job.header, job.rot, job.pos);
        }
        ReleaseCloud(job.dyn_points);
        ReleaseCloud(job.raw_points);
    }
}
