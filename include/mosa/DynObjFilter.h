...
public:
    ...
    bool dyn_points_stable_en = true;
    int dyn_points_accu_limit = 3;
    float dyn_points_voxel = 0.4f;
    std::deque<PointCloudXYZI::Ptr> dyn_points_accu_raw;
    std::deque<PointCloudXYZI::Ptr> dyn_points_accu_cluster_in;
    std::deque<PointCloudXYZI::Ptr> dyn_points_accu_cluster_out;
    int cloud_pool_max = 32;
    std::deque<PointCloudXYZI::Ptr> cloud_pool;
    ...
    bool cluster_async = false;
    int cluster_queue_size = 2;
...
private:
    ...
    PointCloudXYZI::Ptr StabilizeDynPoints(const PointCloudXYZI::Ptr &src,
                                           std::deque<PointCloudXYZI::Ptr> &buffer);
    PointCloudXYZI::Ptr AcquireCloud();
    void ReleaseCloud(PointCloudXYZI::Ptr cloud);
    PointCloudXYZI::Ptr CopyCloudToPool(const PointCloudXYZI::Ptr &src);
    struct ClusterJob
    {
        PointCloudXYZI::Ptr dyn_points;
        PointCloudXYZI::Ptr raw_points;
        std_msgs::msg::Header header;
        M3D rot;
        V3D pos;
    };
    std::mutex cluster_mtx;
    std::condition_variable cluster_cv;
    std::deque<ClusterJob> cluster_queue;
    std::thread cluster_thread;
    bool cluster_thread_running = false;
    void StartClusterThread();
    void StopClusterThread();
    void ClusterWorker();
};
